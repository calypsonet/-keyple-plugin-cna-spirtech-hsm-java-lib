...
/**
 * (package-private)<br>
 * Spirtech HSM plugin extension adapter.
 *
 * @since 2.0.0
 */
final class SpirtechHsmPluginAdapter implements SpirtechHsmPlugin, PoolPluginSpi {

  private static final Logger logger = LoggerFactory.getLogger(SpirtechHsmPluginAdapter.class);
  ...
  public String getName() {
    return SpirtechHsmPluginFactoryAdapter.PLUGIN_NAME;
  }
  ...
  public ReaderSpi allocateReader(String readerGroupReference) throws PluginIOException {
    ...
    SpirtechHsmReaderAdapter hsmReader = null;
    int reference;
    ...
    for (Csm csm : csmList) {
      try {
        ...
        hsmReader = new SpirtechHsmReaderAdapter(csmChannel);
        if (logger.isTraceEnabled()) {
  ...
  public void releaseReader(ReaderSpi readerSpi) throws PluginIOException {
    ...
    try {
      if (readerSpi != null) {
        ((SpirtechHsmReaderAdapter) readerSpi).freeReaderChannel();
        if (logger.isTraceEnabled()) {
